...
//////////////////////////////////////////////////
// Hides or shows forum admin features
//////////////////////////////////////////////////
function forumShowHideAdminFeatures(){
	if(userCheckForumAdmin() || userCheckForumMod()){
		// Check to see what current setting is
		if(isset($_POST['forum_admin_features'])){ $forum_admin_features = $_POST['forum_admin_features']; }else{ $forum_admin_features = ""; }
		if(isset($_SESSION['forum_admin_features_SES'])){ $forum_admin_features_SES = $_SESSION['forum_admin_features_SES']; }else{ $forum_admin_features_SES = ""; }
		
		if($forum_admin_features == 'TRUE'){
			$_SESSION['forum_admin_features_SES'] = "TRUE";
			$redir_link_url = "";
			// Redirect member to their post
			header("Location: $redir_link_url");
			exit;
		}else if($forum_admin_features == 'FALSE'){
			$_SESSION['forum_admin_features_SES'] = "FALSE";
			$redir_link_url = "";
			// Redirect member to their post
			header("Location: $redir_link_url");
			exit;
		...
		// Display buttons based on current setting
		if($forum_admin_features_SES == "TRUE"){
			//echo "Show Admin Features";
			echo "<form method=\"post\" action=\"\" onsubmit=\"submit.disabled = true; return true;\">";
			echo "<input type='hidden' name='forum_admin_features' value='FALSE'>"; 
			//echo "<input type=\"submit\" class=\"sweet\" value=\"Hide Admin Features\" name=\"submit\" onClick=\"this.value = 'Please Wait....'\" />";
			echo "<button type='submit' class='btn btn-success btn-sm' value='Hide Admin Features' name='submit' style='margin-top: 5px; margin-bottom: 0px' data-loading-text='Please wait...' >Hide Admin Features</button>";
			echo "</form>";
		}else{
			//echo "Hide Admin Features";
			echo "<form method=\"post\" action=\"\" onsubmit=\"submit.disabled = true; return true;\">";
			echo "<input type='hidden' name='forum_admin_features' value='TRUE'>"; 
			//echo "<input type=\"submit\" class=\"sweet\" value=\"Show Admin Features\" name=\"submit\" onClick=\"this.value = 'Please Wait....'\" />";
			echo "<button type='submit' class='btn btn-success btn-sm' value='Show Admin Features' name='submit' style='margin-top: 5px; margin-bottom: 0px' data-loading-text='Please wait...' >Show Admin Features</button>";
			echo "</form>";
		}
	}
}
...
//////////////////////////////////////////////////
// Hides or shows forum admin features
// True or false on current setting
//////////////////////////////////////////////////
function forumSHAF(){
	if(userCheckForumAdmin() || userCheckForumMod()){
		// Check to see what current setting is
...
function forumCleanOrderTitle(){
	...
	if(userCheckForumAdmin() || userCheckForumMod()){
		...
		if($result43 = $mysqli->query($query_order)){
			while ($row43 = $result43->fetch_assoc()) {
				$last_row = $row43['forum_order_title'];
			}
			$result43->close();
		}else{
			$last_row = "";
...
function forumCleanOrderCat($forum_title){
	...
	if(userCheckForumAdmin() || userCheckForumMod()){
		...
		if($result22 = $mysqli->query($query_order)){
			while ($row22 = $result22->fetch_assoc()) {
				$last_row = $row22['forum_order_cat'];
			}
			$result22->close();
		}else{
			$last_row = "";
...
function forumEditTitle($f_title){
	...
	if(userCheckForumAdmin() && forumSHAF()){
...
function forumDisplayUserPerms(){
	...
	echo "<div class='well well-sm'>";
		...
		if($forum_perm_level == "Admin" || $forum_perm_level == "Moderator"){
			// Show or hide admin features
			forumShowHideAdminFeatures();
		}
	echo "</div>";
}
		...
		if($AdminCreateCat == "TRUE"){
			...
			echo "<table width='100%'><tr><td>";
				echo "<form enctype=\"multipart/form-data\" action=\"\" method=\"POST\" onsubmit=\"submitmystat.disabled = true; return true;\" class='sweetform' >";
					...
					echo "<input name=\"forum_cat_create\" type=\"text\" value=\"New Category\" style='width:50%;font-family:verdana;font-size:12px;font-weight:bold'><br>";
					echo " <label>Category Description:</label> ";
					echo "<input name=\"forum_des_create\" type=\"text\" value=\"Description\" style='width:75%;font-family:verdana;font-size:12px;font-weight:normal'>";
...
function forumEditCatCheck($f_cat,$f_des,$f_id2){
	...
	if($AdminEditCat == "TRUE"){
		...
		{
			global $websiteUrl, $site_forum_cat;
			echo "<h4><a href='${websiteUrl}${site_forum_title}/forum_display/$f_cat/$f_id2/' title='$f_cat' ALT='$f_cat'>$f_cat</a></h4>";
			echo " - $f_des";
		}
	}
}
